...
class feature_extractor:
    ...
    def statistical_report(self,l):
        ...
        url_match = re.search(r'at\.ua|usa\.cc|baltazarpresentes\.com\.br|pe\.hu|esy\.es|hol\.es|sweddy\.com|myjino\.ru|96\.lt|ow\.ly', l)
        try:
    ...
    def extract(self):
        ...
        # Split the URL into protocol and rest (domain + address)
        seperation_of_protocol = temp_df['URL'].str.split("://", expand=True)
        print("step 1 done")
        print(f"Protocol Separation Output: {seperation_of_protocol}")  # Debug print

        # Split the domain and address, filling missing values with empty strings
        seperation_domain_name = seperation_of_protocol[1].str.split("/", n=1, expand=True).fillna('')
        print(f"Domain and Address Split Output: {seperation_domain_name}")  # Debug print

        # Dynamically assign column names based on the number of columns
        if seperation_domain_name.shape[1] == 1:
            seperation_domain_name.columns = ["domain_name"]
            seperation_domain_name["address"] = ''  # Add empty 'address' column for missing paths
            print("step 3 done: Assigned only domain_name")
        else:
            seperation_domain_name.columns = ["domain_name", "address"]
            print("step 3 done: Assigned both domain_name and address")
